"""
Determine the location by the mode of the probability functions, but add low-pass filter to smoothen the result.
Decrease allowed range of tempo change in tempo re-estimation.
Regularize moving distance between a fixed number of iterations.
"""
...
def get_mode_with_lpf(func):
    kernel = np.full(LPF_SIZE, 1 / LPF_SIZE)
    func = np.convolve(func, kernel)
    return np.argmax(func) - LPF_SIZE // 2
...
class ScoreFollower:
    def __init__(self, config):
        ...
        self._observation_regularization_count = 0
        self._observation_regularization_last_pos = 0

        self._tempo_ub = 1.15 * self._score_tempo
        self._tempo_lb = 0.85 * self._score_tempo
    ...
    def _proc(self, a_time, prev_a_time, a_data, a_input: audio_io.AudioInput):
        ...
        self._cur_pos = get_mode_with_lpf(self._f_source)
        ...
        # observation regularization
        self._observation_regularization_count += 1
        if self._observation_regularization_count == OBSERVATION_REG_INTERVAL:
            self._observation_regularization_count = 0
            if self._cur_pos - self._observation_regularization_last_pos > OBSERVATION_REG_LIMIT:
                self._cur_pos = self._observation_regularization_last_pos + OBSERVATION_REG_LIMIT
            self._observation_regularization_last_pos = self._cur_pos
    ...
    def _compute_f_i_j_given_d(self, time_axis, d, score_tempo, estimated_tempo):
        ...
        mu = math.log(d * rate_ratio) + sigma_square
